...
namespace AssociationWebApp
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddDbContext<AssociationDbContext>(
                options => options.UseSqlServer(Configuration.GetConnectionString("AssociationDbContext"))
            );

            services.AddMvc().SetCompatibilityVersion(CompatibilityVersion.Version_2_1);

            services.AddScoped<AssociationService>();
            services.AddScoped<AssociatedService>();
